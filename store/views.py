    ...
    if 'keyword' in request.GET:
        ...
        if keyword:
            products = Product.objects.order_by(
                '-created_date').filter(Q(decription__icontains=keyword) | Q(product_name__icontains=keyword))
            product_count = products.count()
    context = {
        'products': products,
        'product_count': product_count
...
def submit_review(request, product_id):
    ...
    if request.method == 'POST':
        try:
            ...
            form.save()
            messages.success(
                request, 'Thank you! Your review have been updated')
            return redirect(url)
            ...
            if form.is_valid():
                ...
                data.save()
                messages.success(
                    request, 'Thank you! Your review have been submited')
                return redirect(url)
